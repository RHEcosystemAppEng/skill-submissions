...
description: 'Structured incident investigation for OpenShift using the Five Whys
  methodology, investigation guardrails, Prometheus metric analysis, and adversarial
  due diligence. Orchestrates multi-resource diagnosis across Deployments, ReplicaSets,
  Pods, Services, and cluster resources to trace from observed symptoms to root cause.
  ...
  NOT for single-resource issues with clear patterns (use /debug-pod, /debug-scc,
  /debug-rbac, or /debug-network instead).

  '
license: Apache-2.0
allowed-tools: resources_get resources_list events_list pod_list pod_logs get_metric_names
  get_metric_metadata get_series query
metadata:
  user_invocable: 'true'
---
