...
CRITERIA = [
    {
        "id": "aap_ocp_must_gather_image",
        "file": "/solution/report.md",
        "question": (
            "Does the report provide the correct AAP 2.6 OCP-operator must-gather "
            "command using the version-specific image "
            "registry.redhat.io/ansible-automation-platform-26/aap-must-gather-rhel9, "
            "and does it use the ns-gather command to target a specific namespace?"
        ),
        "reference": (
            "A skilled report uses the exact AAP 2.6 must-gather image "
            "(aap-must-gather-rhel9, not aap-must-gather-rhel8 which is for AAP 2.5) "
            "and includes '-- /usr/bin/ns-gather <namespace>'. An unskilled report uses "
            "generic oc adm must-gather without the AAP-specific image, or uses the "
            "wrong version image (rhel8 instead of rhel9 for AAP 2.6)."
        ),
    },
    {
        "id": "data_sensitivity_warning",
        "file": "/solution/report.md",
        "question": (
            "Does the report include a warning about sensitive data in diagnostic "
            "archives (configuration files, logs, network details, secrets) and "
            "advise the user to review and redact before sharing with Red Hat Support?"
        ),
        "reference": (
            "A skilled report explicitly warns that diagnostic archives contain "
            "comprehensive system data including configuration files, logs, and network "
            "details, and advises reviewing/redacting secrets before sharing. An "
            "unskilled report provides collection commands without any data sensitivity "
            "warning."
        ),
    },
]
