...
CRITERIA = [
  {"id": "remediation_gate", "file": "/solution/report.md", "question": "Does the report gate on remediation availability (checking whether automated remediation is possible for a CVE) before proceeding with playbook generation?", "reference": "A skilled report checks whether automated remediation is available as a prerequisite gate before attempting playbook generation. An unskilled report proceeds to generate playbooks without first verifying that remediation is available for the target CVEs."},
  {"id": "plan_before_execution", "file": "/solution/report.md", "question": "Does the report present a Remediation Plan with summary/table/checklist for user confirmation before execution?", "reference": "A skilled report requires plan validation before execution. An unskilled report executes without plan review."},
  {"id": "two_part_confirmation", "file": "/solution/report.md", "question": "Does the report describe two distinct confirmation checkpoints: one BEFORE starting (upfront planned tasks / Part A) and one AFTER playbook generation but BEFORE execution (execution plan / Part B)?", "reference": "A skilled report has Part A (upfront planned tasks before any remediation step) and Part B (execution plan confirmation after playbook is generated but before running it). An unskilled report has at most one confirmation checkpoint or no structured confirmation phases."}
]
