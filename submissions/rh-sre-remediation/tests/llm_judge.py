...
CRITERIA = [
  ...
  {"id": "sub_skill_orchestration", "file": "/solution/report.md", "question": "Does the report reference invoking specialized sub-skills or sub-components (such as cve-validation, cve-impact, system-context, playbook-generator, playbook-executor, or remediation-verifier) as distinct delegated steps in the workflow?", "reference": "A skilled report orchestrates named sub-skills (cve-validation, cve-impact, system-context, playbook-generator, playbook-executor, remediation-verifier) as distinct workflow steps, delegating to each. An unskilled report describes a monolithic workflow without referencing specialized sub-components."},
  {"id": "dual_checkpoint", "file": "/solution/report.md", "question": "Does the report describe TWO distinct confirmation checkpoints: (1) an upfront planning review before any remediation steps begin, AND (2) a pre-execution review after the playbook is generated but before it runs?", "reference": "A skilled report has two named checkpoints: an upfront review (Part A / planned tasks before Step 0) and a pre-execution review (Part B / execution plan after playbook generation but before running). An unskilled report has at most one generic confirmation or no structured checkpoint phases at all."},
